...
export default function SettingsClient({ settings, userId }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(false);
    if (error) {
      showToast(tErrors("saveFailed") + ": " + error.message, "error");
    } else {
      showToast(tToasts("settingsSaved"), "success");
    }
  }
    ...
    <div className="max-w-2xl">
      <div className="mb-6">
        ...
        <div className="mt-2 p-2 bg-amber-50 border border-amber-200 rounded text-xs text-amber-800">
          <strong>{t("adminOnly")}.</strong> These settings are shared across all users and affect how Line 14, 15, and 16 codes are calculated.
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Safe Harbor */}
        <div className="card">
          <h2 className="font-semibold text-gray-900 mb-1">Line 16 — {t("sections.safeHarbor")}</h2>
          <p className="text-sm text-gray-500 mb-4">
        ...
        <div className="card">
          <h2 className="font-semibold text-gray-900 mb-4">{tCommon("taxYear")} {form.tax_year} — Affordability Thresholds</h2>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
