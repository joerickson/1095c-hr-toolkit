...
export default function PhaseClient({
  ...
  const CATEGORIES = [
    { value: "benefit_setup", label: tIssues("categories.benefit_setup") },
    { value: "eligibility_setup", label: tIssues("categories.eligibility_setup") },
    { value: "company_setup", label: tIssues("categories.company_setup") },
    { value: "employee_data", label: tIssues("categories.employee_data") },
    { value: "dependent_data", label: tIssues("categories.dependent_data") },
    { value: "benefit_assignment", label: tIssues("categories.benefit_assignment") },
    { value: "hours_data", label: tIssues("categories.hours_data") },
    { value: "measurement_period", label: tIssues("categories.measurement_period") },
    { value: "other", label: tIssues("categories.other") },
  ];
  ...
  async function advancePhase() {
    ...
    showToast(t("phaseAdvanced", { current: phaseNumber, next: nextPhase }), "success");
    router.push("/filing");
  }
      ...
      <div className="card">
        ...
        {gateRemaining > 0 && (
          <p className="text-amber-700 text-sm mt-3 flex items-center gap-1.5">
            ...
            <span>
              <strong>{t("gateItemsRemaining", { count: gateRemaining })}</strong> must be checked before you can advance to Phase {phaseNumber < 4 ? phaseNumber + 1 : "completion"}.
            </span>
          </p>
        )}
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {assignments[item.key] && (
                            <p className="text-xs text-gray-400 mt-0.5">{tCommon("assignTo")}: {assignments[item.key]}</p>
                          )}
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="font-medium text-gray-900">
              {phaseNumber < 4
                ? t("advanceToPhase", { next: phaseNumber + 1 })
                : "Complete Filing Process"}
            </p>
            {!gatesComplete && (
              <p className="text-sm text-gray-500 mt-0.5">
                {t("gateItemsRemaining", { count: gateRemaining })} must be checked first
              </p>
            )}
          </div>
          <button
