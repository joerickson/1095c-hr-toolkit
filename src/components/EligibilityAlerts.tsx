  ...
  function buildAlerts(rows: EligibilityDashboardRow[]): Alert[] {
    ...
    if (crossedThreshold.length > 0) {
      alerts.push({
        ...
        message: `${crossedThreshold.length} employee${crossedThreshold.length > 1 ? "s" : ""} ${tPayroll("crossedThreshold")}`,
        severity: "critical",
        href: "/payroll",
      });
    }
    ...
    if (expiredOffers.length > 0) {
      alerts.push({
        id: "offer_expired",
        message: `${expiredOffers.length} offer letter${expiredOffers.length > 1 ? "s" : ""} ${tPayroll("offerExpired")}`,
        severity: "critical",
        href: "/payroll/offers",
      });
    }
    ...
    if (offersNotSent.length > 0) {
      alerts.push({
        id: "offer_not_sent",
        message: `${offersNotSent.length} employee${offersNotSent.length > 1 ? "s" : ""} ${tPayroll("offerNotSent")}`,
        severity: "critical",
        href: "/payroll/offers",
      });
    }
    ...
    if (inAdminPeriod.length > 0) {
      ...
      alerts.push({
        id: "admin_period",
        message: `${tPayroll("inAdminPeriod")} — ${inAdminPeriod.length} employee${inAdminPeriod.length > 1 ? "s" : ""} need offers processed. Coverage must start in ${minDays} day${minDays !== 1 ? "s" : ""}.`,
        severity: "high",
        href: "/payroll/offers",
      });
    }
    ...
    if (soonDeadlines.length > 0) {
      alerts.push({
        id: "offer_deadline",
        message: `${soonDeadlines.length} offer letter${soonDeadlines.length > 1 ? "s" : ""} ${tCommon("deadline").toLowerCase()} approaching within 14 days`,
        severity: "high",
        href: "/payroll/offers",
      });
    }
    ...
    if (approachingThreshold.length > 0) {
      alerts.push({
        ...
        message: `${approachingThreshold.length} employee${approachingThreshold.length > 1 ? "s" : ""} ${tPayroll("approachingThreshold")}`,
        severity: "medium",
        href: "/payroll",
      });
    }
